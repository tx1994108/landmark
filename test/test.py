...
def evaluateError(landmarkGt, landmarkP, bbox):
    e = np.zeros(5)
    for i in range(5):
        e[i] = norm(landmarkGt[i] - landmarkP[i])  #2范数，用来求平方和之后的根号
    e = e / bbox.w
    print 'landmarkGt'
    print landmarkGt
    print 'landmarkP'
...
def E(level=1):
    ...
    error = np.zeros((len(data), 5))  # len(data)为1
    for i in range(len(data)):
        imgPath, bbox, landmarkGt = data[i]
        img = cv2.imread(imgPath, cv2.CV_LOAD_IMAGE_GRAYSCALE)#  把图片变成黑白的
        assert(img is not None)
        logger("process %s" % imgPath)    #函数在utils里面，用来输出当前时间
